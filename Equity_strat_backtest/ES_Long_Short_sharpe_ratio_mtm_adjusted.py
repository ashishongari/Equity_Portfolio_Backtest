...
########################################################################################################################################################
    ...
    for i in range(len(trading_day)):
        ...
        old=live_sharpe_ratio.sort_values(by='sharpe_ratio', ascending=True)

        list_stock=new[:21]
        list_stock_old=old[:4]

        long_short_list=pd.concat([list_stock,list_stock_old])
        stock_list.append(long_short_list.index)
...
#     #ENTRY_PRICE_LIST
    ...
    for i in range(len(trading_day)):
        ...
        price_df=pd.concat([buy_list,df_price], axis=1)
        price_df.columns=['date','price']
        price_df=price_df.dropna()
        price_df=price_df.reset_index(drop=True)
        price_df=price_df['price']

        price_list_buy.append(price_df)
...
#     #EXIT_PRICE_LIST
    ...
    for i in range(len(trading_day)-1):
        ...
        price_list_exit.append(price_df)
